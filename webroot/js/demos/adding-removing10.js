	...
	var reverseToggleDifficulty = false;
	var reverseToggleSize = false;
	var reverseToggleProgress = false;
	var reverseToggleDate = false;
	...
	Demo.prototype.initShuffle = function () {
	  ...
	  reverseToggleDate = false;
	  reverseToggleProgress = false;
	  reverseToggleSize = false;
	  reverseToggleDifficulty = false;
	  this.sortBy('most-solved');
	  
	  /*
	  this.sortBy('most-problems');
	  this.sortBy('most-difficulty');
	  this.sortBy('most-solved');
	  this.sortBy('most-reviews');
	  */
	};
	...
	Demo.prototype.setupEvents = function () {
	  ...
	  document.querySelector('#sorter').addEventListener('click', this.onSortChange.bind(this));
	...
	Demo.prototype.toggleActiveClasses = function (event) {
	  // Add and remove `active` class from buttons. // event.target.value
	...
	Demo.prototype.sortBy = function (value) {
	  ...
	  if (value === 'most-reviews') {
		...
		reverseToggleDate = !reverseToggleDate;
	  } else if (value === 'most-problems') {
		if(reverseToggleSize){
			sortOptions = {
		...
		reverseToggleSize = !reverseToggleSize;
		
	  } else if (value === 'most-difficulty') {
		if(reverseToggleDifficulty){
			sortOptions = {
			   reverse: true,
			  by: this.getDifficulty,
			};
		}else{
			sortOptions = {
			  by: this.getDifficulty,
		...
		reverseToggleDifficulty = !reverseToggleDifficulty;
	  } else if (value === 'most-solved') {
		if(reverseToggleProgress){
			sortOptions = {
		...
		reverseToggleProgress = !reverseToggleProgress;
	...
	Demo.prototype.getProblems = function (element) {
	  return parseInt(element.getAttribute('data-problems'), 10);
	}

	Demo.prototype.getDifficulty = function (element) {
	  return parseInt(element.getAttribute('data-difficulty'), 10);
	}
	
	Demo.prototype.getSolved = function (element) {
	  return parseInt(element.getAttribute('data-solved'), 10);
	}
